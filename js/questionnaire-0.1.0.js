...
console.log('  response:'+JSON.stringify(response));
  ...
  sendMessage: function() {
    if (document.forms['questionnaireForm'].checkValidity()) {
      ...
      var supplier = ractive.get('contact');
      supplier.accountType = 'Direct Supplier';
      supplier.enquiryType = q.name;
      supplier.stage = 'New Questionnaire';
      supplier.tenantId = q.tenantId;
      supplier.account.customFields = quCustomFields; 
      supplier.account.tenantId = q.tenantId;
      console.log('Sending message: '+JSON.stringify(supplier));
      // $('html, body').css("cursor", "wait");
      return $.ajax({
          url: q.submit.endpoint+q.submit.msgName,
          type: 'POST',
          data: {
            json:JSON.stringify(supplier),
